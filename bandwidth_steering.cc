...
std::vector<double> vector_sum(std::vector<double>& a, std::vector<double>& b, int size){
...
int inner_product(std::vector<int>& a, std::vector<int>& b, int size) {
	int sol = 0;
	for (int i = 0; i < size; i++) {
		sol += a[i] * b[i];
	}
...
std::vector<int> matrix_vector_multiply(std::vector< std::vector<int> >& matrix, std::vector<int>& a, int num_rows, int num_cols) {
	std::vector<int> sol;
 ...
 * NOTE: THIS HAS TO BE CALLED BEFORE configure_constraints_cols
 **/
void configure_constraints_rows(int num_groups, 
								optimization_parameters* opt_params, 
								std::string& C_str, 
								std::string& d_str) {
	if (!opt_params)
...
void configure_constraints_cols(int num_groups, 
								optimization_parameters* opt_params, 
								std::string& C_str, 
								std::string& d_str,
								std::vector< std::vector<float> >& traffic_matrix) {
...
void configure_constraints(int num_groups, 
	...
	int num_constraints = 2 * num_groups;
	/*
...
void configure_parameters(int num_groups, optimization_parameters* opt_params, std::vector< std::vector<float> >& traffic_matrix) {
	if (!opt_params) 
		return;
	//opt_params->C = "";
	int n = num_groups * num_groups;
	...
	std::string b_str = "[";
	for (int i = 0; i < n; i++) {
		//std::vector<int>& traffic_row = traffic_matrix[i];
		int row = i / num_groups;
		int col = i % num_groups;
		A_str += "[";
		b_str += "0";
...
void optimize_allocation(int num_groups, optimization_parameters* opt_params, std::vector< std::vector<float> >& traffic_matrix) {
